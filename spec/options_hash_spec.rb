...
describe ::ShellOpts::OptionsHash do
  def process(usage, argv)
    shellopts = ::ShellOpts::ShellOpts.new(usage, argv)
    shellopts.to_hash
  end

  describe "#ast" do
    it "returns the associated Ast object" do
      hash = process("a", %w(-a))
      expect(hash.ast).to eq hash.instance_variable_get("@ast")
    end
  end

  describe "#key" do
    it "returns true iff the option is present" do
      hash = process("a b", %w(-a))
  ...
  describe "#size" do
    it "returns the number of options and commands" do
      hash = process("a,all b", %w(-a -b))
      expect(hash.size).to eq 2
      hash = process("a,all b A!", %w(-a -b A))
  ...
  describe "#values" do
    ...
    it "returns an array of values" do
      hash = process("+a,all= b= c=?", %w(-a ARG1 -a ARG2 -b ARG3 -c))
  ...
  describe "#name" do
    it "returns the used option alias" do
      hash = process("a,all b,bee +c,C", %w(-a --bee -c -C))
    ...
    context "without a name argument" do
      it "returns the name of the AST node of the OptionsHash" do
        hash = process("a", %w(-a))
        expect(hash.name).to eq "rspec"
      end
    end
    ...
    context "with an illegal combination of arguments" do
      it "raises an InternalError" do
        hash = process("a", %w(-a))
        expect { hash.name(nil, 1) }.to raise_error ShellOpts::InternalError
      end
    end
  end
  ...
  describe "#node" do
    it "returns the Ast::Node object" do
      hash = process("a,all +b,bee=", %w(-a -b B --bee=BEE))
      expect(hash.node("-a").grammar).to be hash.ast.grammar.options["-a"]
    end
    ...
    context "without a name argument" do
      it "returns the AST node for the OptionsHash" do
        hash = process("a", %w(-a))
        expect(hash.node).to eq hash.ast
      end
    end

    context "with an index argument" do
      it "returns the node at that index" do
        hash = process("+b,bee=", %w(-b B --bee=BEE))
    ...
    context "with an illegal combination of arguments" do
      it "raises an InternalError" do
        hash = process("a", %w(-a))
        expect { hash.node(nil, 1) }.to raise_error ShellOpts::InternalError
      end
    end
  end
